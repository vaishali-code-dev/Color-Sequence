     ...
     {arr.map((item, index)=> {
      return(     
        <div className="boxWrapper" key = {index}>
         { item.map((child, subIndex)=> 
         child===1 && 
         <Box key ={`${index}-${subIndex}`} id ={`${index}-${subIndex}`} onClick={handleSubmit.bind(this, `${index}-${subIndex}`)} 
         customClassName={res.includes(`${index}-${subIndex}`) && "blue"}/>
)}
        </div>
